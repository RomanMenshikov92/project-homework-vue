<template>
  <div
    class="project-details__slider"
    @mouseover="showIcon"
    @mouseleave="hideIcon"
  >
    <div
      class="project-details__slider-wrapper"
      :class="{ large: showLargeImage }"
    >
      <img
        class="project-details__slider-wrapper-img"
        :class="{ show: img.id === currentImg }"
        v-for="img in imgSlider"
        :key="img.id"
        :src="img.src"
        alt="img"
        @click="hideLargeImg"
      />
      <div
        class="project-details__slider-wrapper-search"
        v-if="showSearchIcon"
        @click.stop="showLargeImg"
      >
        <svg
    ...
    <div class="project-details__slider-pagination">
      <div
        class="project-details__slider-pagination-element"
        @click="clickPagination(i)"
        v-for="i in imgSlider.length"
        :key="i.id"
        :class="{ current: i === currentImg }"
      ></div>
    </div>
  </div>
</template>
...
@import './src/assets/sass/components/_slider.sass';
</style>
